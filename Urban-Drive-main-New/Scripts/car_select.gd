...
var car_prices := {
	# COMPACTS
	"Kuro Zephyr": 28000,
	"Schroder Atrix Q32": 32000,
	"Zenith Horizon": 36000,
	"Straeda B32": 30000,
	"Schroder D-20": 24000,

	# MUSCLE
	"Brutus Viper": 55000,
	"Mir Cars Hutch": 68000,

	# URBAN
	"Kuro Serenity": 72000,
	"Eisenach Bengal": 52000,
	"Strandberg Turbo": 60000,
	"Berkshire Blunt": 78000,
	"Kronstadt Essence": 76000,
	"Eisenach Prince": 70000,
	"Kestrel Speedster": 88000,

	# SEDANS
	"Kuro Vault": 78000,
	"Eisenach Suppressor": 58000,
	"Mir Cars Transporter": 90000,
	"Kronstadt Fortress": 110000,
	"Kuro Persian": 68000,

	# SUVs
	"Colossus Titan Max": 40000,
	"Colossus Behemoth": 0,
	"Schroder Colosso": 60000,
	"Mir Cars Nightwolf": 48000,

	# SPORT
	"Eisenach Goblin": 135000,
	"Schroder Classique Sport": 115000,
	"Brutus Stingray": 125000,
	"Berkshire V12-S": 150000,
	"Berkshire Tempest": 160000,
	"Kestrel Touring": 145000,

	# SPORT RACING
	"Linetti Shepherd": 225000,
	"Schroder Atrocity": 205000,
	"Brutus Venom": 185000,
	"Kestrel Battleaxe": 240000,

	# SUPERCARS
	"Kestrel Guillotine": 350000,
	"Linetti Terror": 500000,
	"Mir Cars Raptor": 480000,
	"Linetti Firestorm": 440000,

	# TRACK / HYPERCARS
	"Mir Cars Athletic C70": 820000,
	"Bartoli Track Cruiser": 1_050_000,
	"Brutus Thunderbolt": 900000
}
...
func _ready():
	...
	if dealership_mode:
		...
		$Control/CarStats/PriceLabel.visible=true
	else:
		$Select.text = "SELECT"
...
func get_available_owned_cars(cup_id: String) -> Array[String]:
	var all := ClubCups.get_available_owned_cars(cup_id)
	var owned := []
...
func _get_filtered_list(raw_list: Array) -> Array:
	...
	var allowed : Array[String] = ClubCups.get_available_owned_cars(cup_id)

	var filtered : Array = []
	for car_name in raw_list:
		if allowed.has(car_name) and Unlocks.is_unlocked(car_name):
...
func update_car_ui(stats: Array, name: String):
	...
	$Control/CarStats/PriceLabel.text = "Price: $" + str(car_prices.get(name, 0))
...
func _on_select_pressed():
	if dealership_mode:
		...
		# Already owned → show label, keep button hidden only for this car
		if Unlocks.is_unlocked(car_name):
			...
			$Select.visible = true
			$Control/Label.text = ""  # clear message if not owned

		# Not enough money → show label, don’t hide button
		if Cars.money < price:
			$Control/Label.text = "NOT ENOUGH MONEY!"
			$Control/Label.modulate = Color.RED
			return
		...
		Cars.money -= price
		Cars.save_money()
		Unlocks.unlock_car(car_name)
		print("Purchased:", car_name)

		_update_money_label()

		# After purchase, show "ALREADY OWNED!" and hide BUY for this car
		$Control/Label.text = "ALREADY OWNED!"
		$Control/Label.modulate = Color.RED
		$Select.visible = false
		return
		...
		Cars.selected_car = car_scene_paths.get(car_name, "")
	...
	RoadChallengeState.active_color = car_colors.get(car_name, [])[color_index]
...
func _on_track_cars_pressed():
	...
	# Helper function to update each class button
func _update_class_locks():
	if dealership_mode:
		# In dealership mode, all class buttons are enabled
			$Control/ClassList/SUV.disabled = false
			$Control/ClassList/CompactCars.disabled = false
			$Control/ClassList/MuscleCars.disabled = false
			$Control/ClassList/UrbanRacers.disabled = false
			$Control/ClassList/Sedans.disabled = false
			$Control/ClassList/SportCoupe.disabled = false
			$Control/ClassList/SportRacing.disabled = false
			$Control/ClassList/Supercars.disabled = false
			$Control/ClassList/TrackCars.disabled = false
			return
	# Only apply locks in Club Cups
	if GameMode.game_mode != "Club Cups":
		return
	...
	_apply_class_lock("suv", suv_list, allowed)
	_apply_class_lock("compact", compact_list, allowed)
	_apply_class_lock("muscle", muscle_list, allowed)
	_apply_class_lock("urban", urban_list, allowed)
	_apply_class_lock("sedans", sedans_list, allowed)
	_apply_class_lock("sport", sport_list, allowed)
	_apply_class_lock("sport_racing", sport_racing_list, allowed)
	_apply_class_lock("supercars", supercars_list, allowed)
	_apply_class_lock("track_cars", track_cars_list, allowed)

func _class_has_any(raw_list: Array, allowed_list: Array) -> bool:
	for car_name in raw_list:
		if allowed_list.has(car_name) and Unlocks.is_unlocked(car_name):
			return true
	return false


func _apply_class_lock(class_id: String, raw_list: Array, allowed_list: Array):
	var btn_name: String

	match class_id:
		"suv": btn_name = "SUV"
		"compact": btn_name = "CompactCars"
		"muscle": btn_name = "MuscleCars"
		"urban": btn_name = "UrbanRacers"
		"sedans": btn_name = "Sedans"
		"sport": btn_name = "SportCoupe"
		"sport_racing": btn_name = "SportRacing"   # FIXED
		"supercars": btn_name = "Supercars"
		"track_cars": btn_name = "TrackCars"       # FIXED
		_: return   # unknown class → skip

	var btn: Button = $Control/ClassList.get_node(btn_name)

	var has_any: bool = _class_has_any(raw_list, allowed_list)

	btn.disabled = not has_any

	if has_any:
		btn.modulate = Color(1, 1, 1)
	else:
		btn.modulate = Color(0.5, 0.5, 0.5)
func toggle_dealership_mode():
	dealership_mode = not dealership_mode

	if dealership_mode:
		$Control/SelectButton.text = "BUY"
		$Control/ColorSelector.visible = false
	else:
		$Control/SelectButton.text = "SELECT"
		$Control/ColorSelector.visible = true
func get_car_price(name: String) -> int:
	return car_prices.get(name, 50000)
