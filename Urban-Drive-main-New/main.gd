...
func show_finish(player_won: bool):
	...
	# ============================================================
#  CLUB CUPS CAREER PROGRESSION (FULL 4-MODE SYSTEM)
# ============================================================
	if GameMode.game_mode == "Club Cups" and player_won:
		var current : String = ChampionshipState.active_cup

		# Determine which mode was completed
		var mode_id := ""
		if mode == "Duel":
			mode_id = "duel"
		elif mode == "Radar Race":
			mode_id = "radar"
		elif mode.to_lower() == "normal race":
			mode_id = "normal"
		elif mode == "Elimination":
	...
	# Mark mode completed
		if mode_id != "":
			ClubCups.mark_mode_completed(current, mode_id)
			print("Completed", mode_id, "for cup:", current)

		# Check if cup is fully completed (all 4 modes)
		if ClubCups.is_cup_completed(current):
			print("Cup fully completed:", current)

			# Unlock next cup
			var idx := ClubCups.career_order.find(current)
		...
		# Unlock reward car
		if ClubCups.cup_rewards.has(current):
			var reward : String = ClubCups.cup_rewards[current]
	...
	if player_won:
		Cars.money += 4000
		Cars.save_money()
		print("Money earned: $4000")

	if GameMode.game_mode == "Road Challenge" and player_won:
		var class_id := Cars.selected_class
		RoadChallengeSave.progress[class_id] += 1
		RoadChallengeSave.unlock_next_group(class_id)
		RoadChallengeSave.save()
	# ============================================================
#  CLUB CUPS CAREER PROGRESSION
# ============================================================
	if GameMode.game_mode == "Club Cups" and player_won:
		var current := ChampionshipState.active_cup

		# Unlock next cup in career_order
		var idx := ClubCups.career_order.find(current)
		if idx != -1 and idx < ClubCups.career_order.size() - 1:
			var next :String= ClubCups.career_order[idx + 1]
			ClubCups.unlocked_cups[next] = true
